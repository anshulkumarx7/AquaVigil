...
const EmployeeList = ({ setEmployeeListView }) => {
  ...
  const handleFilteredEmployees = (idx) => {
    if (!addedEmployees.includes(employees[idx]))
      setAddedEmployees((prevAddedEmployees) => [
        ...prevAddedEmployees,
        employees[idx],
      ]);
  ...
  const handleRemove = (idx) => {
    setAddedEmployees((previousAddedEmployees) =>
      previousAddedEmployees.filter(
        (employee, index) => employee._id !== employees[idx]._id
      )
    );
  };
    ...
    <div className="w-[79vw] h-[80.0vh] flex flex-col gap-2 justify-center bg-white rounded-lg">
      ...
      <div className="w-[76vw] flex flex-col gap-[8px] h-[75vh] font-semibold overflow-y-scroll overflow-x-hidden">
        {employees.map((employee, index) => (
          <div
            key={index}
            className="flex h-[5vh] text-[#3A4264] justify-evenly items-center w-[76vw]"
          >
            <p className="w-[10.17%] flex items-center justify-start">{`E${
              index + 1
            }`}</p>
            ...
            {addedEmployees.includes(employee) ? (
              <p
                className="w-[10.17%] flex items-center justify-start gap-2"
                onClick={() => handleRemove(index)}
              >
                <div
                  className={`h-[24px] w-[24px] rounded-full bg-[hsl(0,55%,50%)]`}
                ></div>
                Remove
              </p>
            ) : (
              <p className="w-[10.17%] flex items-center justify-start gap-2">
                <div
                  className={`h-[24px] w-[24px] rounded-full bg-[#05CD99]`}
                ></div>
                Available
              </p>
            )}
            ...
            <div
              ...
              className="w-[10.35%] cursor-pointer flex items-center justify-center border-[1px] text-[#234DF0] rounded-md border-[#234DF0]"
            >
              Assign
            </div>
          </div>
        ...
        <button
          ...
          className="w-[10.35%] mx-auto cursor-pointer flex items-center justify-center border-[1px] text-[#fff] rounded-md bg-[#234DF0]"
        >
          Assign
        </button>
